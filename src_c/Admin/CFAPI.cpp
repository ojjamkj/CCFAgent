...
struct rootNode* CFAPI::addNode(struct rootNode** head, bool _isFile, char *_filePath) {
	struct FileNode* newNode = (struct FileNode*)malloc(sizeof(struct FileNode));  // 새로운 노드 동적 할당
	strcpy(newNode->filePath, _filePath);
	newNode->isFile = _isFile;
...
void CFAPI::listFiles(struct FileNode **node,const char* path, const char* regexPattern) {
...
void CFAPI::API27_DOSEARCH_ONLY_FILE(CBRMObj  *m_ObjBuffer2, int m_itemCnt, int pChildSoc, CCSManager *pManage)
{
	...
	listFiles( &fileList, path , regexPattern);
